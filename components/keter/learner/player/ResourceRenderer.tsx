...
// Keter — Anderson resource types created by lib/keter/mockAIAgent.ts. The
// original Courses module never produces slides / narrated-walkthrough /
// knowledge-check Resources, so the original ResourceRenderer doesn't have
// cases for them; the Keter clone needs them or every Anderson lesson section
// other than text falls into the "Unknown resource type" default.
import SlidePlayer from "./SlidePlayer";
import NarrationPlayer from "./NarrationPlayer";
import KnowledgeCheckPlayer from "./KnowledgeCheckPlayer";
  ...
  const renderResource = () => {
    try {
      switch (resource.type) {
        ...
        case "slides":
          return <SlidePlayer slides={resource.slides || []} />;
        case "narrated-walkthrough":
          return resource.narrationData ? (
            <NarrationPlayer narrationData={resource.narrationData} />
          ) : (
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                Narrated walkthrough has no narration data.
              </p>
            </div>
          );
        case "knowledge-check":
          return resource.knowledgeCheckData ? (
            <KnowledgeCheckPlayer data={resource.knowledgeCheckData} />
          ) : (
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                Knowledge check has no question data.
              </p>
            </div>
          );
        default:
