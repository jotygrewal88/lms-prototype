// Keter — Anderson module: dedicated in-memory store (parallel to lib/store.ts)
// Owns Keter-only courses, lessons, resources, quizzes, assignments, progress,
// certificates, undo/redo, audit, library, changeLog, synthesis history.
// Reads shared user/role/site/department/scope state from @/lib/store so that
// role-switching done in the Header propagates into Keter pages.
"use client";
...
import {
  keterCourses as seedCourses,
  keterLessons as seedLessons,
  keterResources as seedResources,
  keterQuizzes as seedQuizzes,
  keterQuestions as seedQuestions,
  keterAssignments as seedAssignments,
  keterProgressCourses as seedProgressCourses,
  keterProgressLessons as seedProgressLessons,
  keterCertificates as seedCertificates,
} from "@/data/keter/seedCourses";
...
import { keterLibraryItems as seedLibraryItems } from "@/data/keter/seedLibrary";
...
// Re-export current-user state and main subscribe from the shared store so that
// Header role-switching propagates into Keter pages.
import {
  getCurrentUser as getMainCurrentUser,
  switchRole as mainSwitchRole,
  subscribe as mainSubscribe,
} from "@/lib/store";

// Re-export Scope type for convenience
export type { Scope };

// In-memory state (currentUser is owned by main store; do not mirror here)
...
// Keter Course Library state (no LOTO seed; Keter has its own seed)
...
// Listeners for Keter-state changes
type Listener = () => void;
...
// Keter subscribe also forwards to the main store so Keter components react to
// role-switches and other shared-state changes triggered from the Header.
export function subscribe(listener: Listener): () => void {
  listeners.push(listener);
  const unsubscribeMain = mainSubscribe(listener);
  return () => {
...
// Current user management — proxied to main store so the Header role-selector
// (which writes to main store) is the single source of truth.
export function getCurrentUser(): User {
  return getMainCurrentUser();
}

export function switchRole(userId: string): void {
  mainSwitchRole(userId);
}
...
export function getQuestionsByQuizId(quizId: string): Question[] {
  ...
  // Canonical model (per types.ts:541) is `Quiz.questions: Question[]` —
  // `questionIds` is marked deprecated/legacy. The Keter wizard, the LOTO
  // seed (data/seedLOTOCourse.ts), and the editor's createLessonsFromOutline
  // all write to `quiz.questions` directly without ever populating
  // `questionIds` or pushing into the global `questions` array. The original
  // helper only reads `questionIds`, so any quiz created by those code paths
  // appears empty to surfaces that go through this getter (notably the
  // preview surface PreviewQuizPanel). The Keter clone falls back to the
  // canonical inline `quiz.questions` array first; only when that's empty
  // does it use the legacy questionIds + global questions lookup.
