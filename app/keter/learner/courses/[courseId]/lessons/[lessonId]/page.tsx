...
import CoursePlayerHeader from "@/components/keter/learner/CoursePlayerHeader";
import CoursePlayerSidebar from "@/components/keter/learner/CoursePlayerSidebar";
import LessonContentRenderer from "@/components/keter/learner/LessonContentRenderer";
import CoursePlayerFooter from "@/components/keter/learner/CoursePlayerFooter";
import CourseCompletionSummary from "@/components/keter/learner/CourseCompletionSummary";
import LessonTimer from "@/components/keter/learner/LessonTimer";
import VideoProgressTracker from "@/components/keter/learner/VideoProgressTracker";
...
export default function CoursePlayerLessonPage() {
  ...
  useEffect(() => {
    ...
    if (!lesson) {
      ...
      if (progressCourse?.lastLessonId) {
        router.replace(`/keter/learner/courses/${courseId}/lessons/${progressCourse.lastLessonId}`);
      } else if (lessonsData.length > 0) {
        router.replace(`/keter/learner/courses/${courseId}/lessons/${lessonsData[0].id}`);
      } else {
        router.push("/learner");
      }
      return;
    ...
    if (!isLessonUnlocked(courseData, lessonId, getCurrentUser().id)) {
      ...
      if (unlockedLesson) {
        router.replace(`/keter/learner/courses/${courseId}/lessons/${unlockedLesson.id}`);
        return;
      }
    }
  ...
  const handleLessonClick = useCallback(
    (newLessonId: string) => {
      ...
      router.push(`/keter/learner/courses/${courseId}/lessons/${newLessonId}`);
    },
  ...
  if (showCompletionSummary) {
    return (
      <KeterRouteGuard allowedRoles={["LEARNER"]}>
      ...
      <div className={`flex flex-col h-screen ${highContrast ? "bg-white" : "bg-gray-50"}`}>
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-2">
          <div className="max-w-full mx-auto flex items-center gap-2 text-sm text-gray-600">
            <Link href="/learner" className="hover:text-gray-900">Back to Courses</Link>
            <span>/</span>
            <Link href={`/keter/learner/courses/${courseId}`} className="hover:text-gray-900">{course.title}</Link>
            <span>/</span>
        ...
        <CoursePlayerHeader
          ...
          onResume={(resumeLessonId) => router.push(`/keter/learner/courses/${courseId}/lessons/${resumeLessonId}`)}
          textSize={textSize}
