...
import ResourceRenderer from "@/components/keter/learner/player/ResourceRenderer";
...
export default function CoursePreviewPage() {
  ...
  useEffect(() => {
    ...
    // Resolve the quiz that should display on this lesson:
    //   - Lesson-level quiz if one is attached (matches LOTO behavior, where
    //     each lesson has its own knowledge-check quiz).
    //   - Course-level quiz ONLY when the lesson is an assessment lesson
    //     (lessonType === "assessment"). Without that guard, every Anderson
    //     lesson would fall back to the course-level Final Assessment quiz
    //     because lessons 1–5 don't have their own lesson-level quizzes
    //     (their knowledge checks are knowledge-check Resource sections, not
    //     quiz entities).
    const allQuizzes = getQuizzesByCourseId(courseId);
    const lessonQuiz = allQuizzes.find(q => q.lessonId === currentLesson.id);
    const isAssessment = currentLesson.lessonType === "assessment";
    const courseQuiz = allQuizzes.find(q => !q.lessonId);
    const activeQuiz = lessonQuiz || (isAssessment ? courseQuiz : undefined);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <PreviewHeader 
        ...
        onExit={() => router.push(`/keter/admin/courses/${courseId}/edit`)}
      />
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="max-w-4xl mx-auto px-8 py-8">
            ...
            <div className="space-y-8">
              {resources.length === 0 && !(quiz && questions.length > 0) ? (
                // Empty state only when there are neither Resources nor a
                // quiz attached to this lesson. The Anderson Final Assessment
                // lesson has no Resources but does have a course-level quiz
                // — for that lesson we skip the empty state and render the
                // quiz panel directly (see Quiz Section below).
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {currentLessonIndex < lessons.length - 1 ? (
                    ...
                    <Button
                      variant="primary"
                      onClick={() => router.push(`/keter/admin/courses/${courseId}/edit`)}
                      className="flex items-center gap-2"
                    >
                      Finish Preview
                      <CheckCircle2 className="w-4 h-4" />
